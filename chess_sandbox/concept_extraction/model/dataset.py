...
def rebalance_positions(positions: list[LabelledPosition], random_state: int = 42) -> list[LabelledPosition]:
    ...
    Example:
        ...
        >>> set(p.move_number for p in balanced)
        {1, 2, 3, 5}
    """
    random.seed(random_state)

    positions_with_concepts, positions_without_concepts = split_positions(positions)
...
def split_positions(positions: list[LabelledPosition]) -> tuple[list[LabelledPosition], list[LabelledPosition]]:
    positions_with_valid_concepts: list[LabelledPosition] = []
    ...
    for pos in positions:
        if pos.concepts:
            ...
            if validated_concepts:
                positions_with_valid_concepts.append(pos)
            else:
